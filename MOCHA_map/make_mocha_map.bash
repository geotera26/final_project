...
# Position of distance scale
distScalePos=-128.2/46.5/-128.6/46.1/100
...
cat MOCHA_sitelab.txt | grep -e 'B' | psxy -R -J $col_select $stat_pen -G$Bcol $stats -O -K >> $filename.ps
cat MOCHA_sitelab.txt | grep -e 'B' | pstext ${stat_font}${Bcol},-=$outline $stat_font_offset $scale -R -O  -K >> $filename.ps


cat MOCHA_sitelab.txt | grep -e 'D' | psxy -R -J $col_select $stat_pen -G$Dcol $stats -O -K >> $filename.ps
cat MOCHA_sitelab.txt | grep -e 'D' | pstext ${stat_font}$Dcol,-=$outline $stat_font_offset $scale -R -O  -K >> $filename.ps

cat MOCHA_sitelab.txt | grep -e 'F' | psxy -R -J $col_select $stat_pen -G$Fcol $stats -O -K >> $filename.ps
cat MOCHA_sitelab.txt | grep -e 'F' | pstext ${stat_font}$Fcol,-=$outline $stat_font_offset $scale -R -O  -K >> $filename.ps

cat MOCHA_sitelab.txt | grep -e 'H' | psxy -R -J $col_select $stat_pen -G$Hcol $stats -O -K >> $filename.ps
cat MOCHA_sitelab.txt | grep -e 'H' | pstext -F+f6.8p,Helvetica-Bold,$Hcol,-=$outline $stat_font_offset $scale -R -O  -K >> $filename.ps

cat MOCHA_sitelab.txt | grep -e 'J' | psxy -R -J $col_select $stat_pen -G$Jcol $stats -O -K >> $filename.ps
cat MOCHA_sitelab.txt | grep -e 'J' | pstext ${stat_font}$Jcol,-=$outline $stat_font_offset $scale -R -O  -K >> $filename.ps

cat MOCHA_sitelab.txt | grep -e 'L' | psxy -R -J $col_select $stat_pen -G$Lcol $stats -O -K >> $filename.ps
cat MOCHA_sitelab.txt | grep -e 'L' | pstext -F+f6.8p,Helvetica-Bold,$Lcol,-=$outline $stat_font_offset $scale -R -O  -K >> $filename.ps

cat MOCHA_sitelab.txt | grep -e 'N' | psxy -R -J $col_select $stat_pen -G$Ncol $stats -O -K >> $filename.ps
cat MOCHA_sitelab.txt | grep -e 'N' | pstext ${stat_font_s}$Ncol,-=$outline $stat_font_offset $scale -R -O  -K >> $filename.ps

cat MOCHA_sitelab.txt | grep -e 'P' | psxy -R -J $col_select $stat_pen -G$Pcol $stats -O -K >> $filename.ps
cat MOCHA_sitelab.txt | grep -e 'P' | pstext ${stat_font_s}$Pcol,-=$outline $stat_font_offset $scale -R -O  -K >> $filename.ps

# echo " appending distance scale..."
# pscoast $scale $region  -Lf$distScalePos+l"km"  -W1/0.25p,black -O -K >> $filename.ps
